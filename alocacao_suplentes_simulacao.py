...
# 1) CONFIGURAÇÕES
# ==============================================================================
APPSHEET_DB_ID = '1I-mD7tLgo7_ZrPf58MEeZ-bRm0cltKu2_-XssMJcRsI' 
SPREADSHEET_ID = APPSHEET_DB_ID
ABA_PESSOAS = "Pessoas"
...
# ==============================================================================
def get_google_sheet_data(sheet_name, spreadsheet_id=None):
    ...
    # SEGREDO: Se passar ID, usa ele. Se não, usa o global (do usuário atual)
    target_id = spreadsheet_id if spreadsheet_id else SPREADSHEET_ID

    try:
        range_ = f"{sheet_name}!A1:Z"
        result = service.spreadsheets().values().get(spreadsheetId=target_id, range=range_).execute()
...
#  FUNÇÕES SIMULAÇÃO
# =================================================================================
def ler_fila_simulacao_appsheet():
    # Lê a aba onde estão os pedidos de simulação (a mesma da imagem que você mandou)
    df_fila = get_google_sheet_data("Otimizar Alocação de Suplentes") 
    
    # Filtra apenas quem está com Status "Pendente"
    fila = df_fila[df_fila["Status_Simulacao"] == "Pendente"]
    return fila

def atualizar_linha_appsheet(row_index, status, link_relatorio=None, log_erro=None):
    creds = get_bot_creds()
    service = build('sheets', 'v4', credentials=creds)
    
    linha_excel = row_index + 2 
    
    # ID fixo da planilha do AppSheet
    APPSHEET_DB_ID = '1I-mD7tLgo7_ZrPf58MEeZ-bRm0cltKu2_-XssMJcRsI' 
    NOME_ABA = "Otimizar Alocação de Suplentes"
    
    updates = []
    
    # Status (Coluna E)
    updates.append({'range': f"'{NOME_ABA}'!E{linha_excel}", 'values': [[status]]})
    
    if link_relatorio:
        # Link (Coluna F)
        updates.append({'range': f"'{NOME_ABA}'!F{linha_excel}", 'values': [[link_relatorio]]})
        
    # Se houver erro, apenas avisamos no console do Python (já que não vai pra planilha)
    if log_erro:
        print(f"   [ALERTA] Erro na linha {linha_excel} (não salvo na planilha): {log_erro}")

    body = {'valueInputOption': 'USER_ENTERED', 'data': updates}
    service.spreadsheets().values().batchUpdate(spreadsheetId=APPSHEET_DB_ID, body=body).execute()
    print(f"   [APPSHEET] Linha {linha_excel} atualizada para: {status}")


# Função auxiliar simples para extrair ID
def extrair_id_do_google_drive(url):
...
def processar_simulacao(id_planilha_input, id_linha_appsheet):
    ...
    if res_pes or res_proj or res_sobras:
        out = f"Resultado_Simulacao_{id_linha_appsheet}.xlsx"
        try:
